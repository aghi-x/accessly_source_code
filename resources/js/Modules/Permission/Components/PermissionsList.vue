...
function goToEdit(permissionID) {
  router.visit(`/permissions/${permissionID}/edit`)
}

function goToDetails(permissionID) {
  router.visit(`/permissions/${permissionID}`)
}
...
<template>
  <div class="overflow-x-auto bg-base-100 shadow-md rounded-lg">
    <table class="table w-full">
      ...
      <!-- Table Body -->
      <tbody>
        <tr v-for="permission in permissions" :key="permission.id" class="hover">
          <td class="font-medium">{{ permission.name }}</td>

          <td>
            <div class="flex justify-center items-center gap-2">
              <button
                @click="goToEdit(permission.id)"
              ...
              <button
                @click="goToDetails(permission.id)"
                class="btn btn-square btn-ghost"
                title="Details"
              >
